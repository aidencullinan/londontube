...
public class LondonTube {
	...
	public static void main(String[] args) throws IllegalArgumentException {
		...
		System.out.println("London Tube - Aiden Cullinan\n");
	...
	public static void printPath(Graph<Station, LineSegment> g, PositionalList<Edge<LineSegment>> path,
		...
		Vertex<Station> next;
		Edge<LineSegment> line = null;
		Vertex<Station> lineStart = origin;
		for (Edge<LineSegment> e : path) {
			...
			else if (line.getElement().getLine().getName() != e.getElement().getLine().getName()) {
				System.out.printf("Take the %s line from %s to %s\n", line.getElement().getLine().getName(),
						lineStart.getElement().getName(), current.getElement().getName());
				System.out.println("Switch to the " + e.getElement().getLine().getName() + " line at " + current.getElement().getName());
				lineStart = current;
				line = e;
			}
	...
	static public Vertex<Station> findStation(Graph<Station, LineSegment> g, String name) {
		for (Vertex<Station> v : g.vertices()) {
			if (v.getElement().getName().replaceAll(" ", "").replaceAll("\\p{Punct}", "").equalsIgnoreCase(name.replaceAll(" ", "").replaceAll("\\p{Punct}", "")))
				return v;
		}
		return null;
	}
